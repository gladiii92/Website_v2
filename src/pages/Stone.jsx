...
import { ArrowLeft, Play, Pause, Share2, Award, Shield, Sparkles, ChevronLeft, ChevronRight } from "lucide-react";
...
const translations = {
  de: {
    ...
    tanzanite: "Tansanit",
    spinel: "Spinell",
    // Erweiterung für Cuts, Origins und Colors
    cuts: {
      oval: "Oval",
      cushion: "Kissen",
      brilliant: "Brillant",
    ...
    colors: {
      ...
      gruen: "Grün",
      rot: "Rot",
      // Füge mehr Farben hinzu, z. B. pink: "Rosa"
    },
    certifications: {
      aig: "AIG-Zertifikat",
      igi: "IGI-Zertifikat",
...
export default function StonePage() {
  ...
  const loadStone = async (stoneId) => {
    try {
    const stoneData = gemstones.find(s => String(s.id) === String(stoneId));
    if (stoneData) {
      setStone(stoneData); 
  ...
  // Neue Funktion (füge nach getTranslatedColor ein)
  const getTranslatedStoneName = (id) => {
    return t.stone_names?.[id] || stone.name;  // Fallback auf originalen Namen aus Gemstones.js
  };
  ...
  const allMedia = [stone.main_image_url, ...(stone.gallery_images || [])].filter(Boolean);
          ...
          <div className="space-y-4">
            <div className="relative aspect-square bg-gray-50 rounded-lg overflow-hidden group">
              {allMedia[currentIndex].endsWith('.mp4') ? (
                ...
                <img
                  src={allMedia[currentIndex] || "https://images.unsplash.com/photo-1573408301185-9146fe634ad0?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1000&q=80"}
                  alt={stone.name}
              ...
              {allMedia.length > 1 && (
                <>
                  <button
                    onClick={() => setCurrentIndex((prev) => (prev - 1 + allMedia.length) % allMedia.length)}
            ...
            {/* Thumbnail navigation – Falls du es entfernen möchtest, kommentiere diesen Block aus */}
            {allMedia.length > 1 && (
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-xl text-gray-600">
                    {getTranslatedType(stone.type)} • {stone.carat_weight}ct • {getTranslatedCut(stone.cut)} • {getTranslatedOrigin(stone.origin)} • {getTranslatedTreatment(stone.treatment)}
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    {stone.rarity_level && (
                      <div className="border-b border-gray-100 pb-3">
                        ...
                        <dd className="text-gray-900 font-semibold">{getTranslatedRarity(stone.rarity_level)}</dd>
                      </div>
                    )}
                    <div className="border-b border-gray-100 pb-3">
